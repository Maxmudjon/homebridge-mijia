...
MiJiaPlatform.prototype.parseMessage = function(msg, rinfo){
  ...
  if (cmd === 'iam') {
    ...
    //console.log(battery);
    fs.writeFile("/usr/local/lib/node_modules/homebridge-mijia/"+id+".json", JSON.stringify(battery), 'utf-8', function(err) {
	if (err) throw err
...
TemperatureAndHumidityParser.prototype.parse = function(report) {
  ...
  fs.readFile("/usr/local/lib/node_modules/homebridge-mijia/"+deviceSid+".json", 'utf-8', function(err, data) {
  if (err) throw err
  var battery = JSON.parse(data)
  })
  var battery = fs.readFileSync("/usr/local/lib/node_modules/homebridge-mijia/"+deviceSid+".json", 'utf8');  
  if (battery >= 2801){
  	battery = 0;
  } else {
  	battery = 1;
  ...
  this.factory.setTemperatureAndHumidity(gatewaySid, deviceSid, temperature, humidity, battery);
}
...
MotionParser.prototype.parse = function(report, rinfo) {
  ...
  this.factory.setMotion(gatewaySid, deviceSid, motionDetected, battery);
}
...
ContactParser.prototype.parse = function(report, rinfo) {
  ...
  this.factory.setContact(gatewaySid, deviceSid, contacted, battery);
}
...
WirelessSwitchParser.prototype.parse = function(report, rinfo) {
  ...
  this.factory.setWirelessSwitch(gatewaySid, deviceSid, status, battery);
}
